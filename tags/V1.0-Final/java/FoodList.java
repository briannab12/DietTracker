...
public class FoodList extends Observable{
    ...
    public void loadFoodData(){
        ...
        for(String[] foodArray : foodStrings) {
            ...
            if (foodArray[0].equals("r")){
                //holds all the basic foods in a recipe
                ArrayList<Food> recipeFoods = new ArrayList<Food>();
                //for each basic food in a recipe, create new food obj and set servings
                for (int i = 2; i < foodArray.length; i+=2) {
                    Food basicFood = foodList.get(foodArray[i]);
                    Double servings = Double.parseDouble(foodArray[i+1]);
                    Food foodClone = BasicFood.clone(basicFood, servings);
    ...
    public Food getFood(String name){
        return new BasicFood("duh", 5, 5, 5, 5, 5);
    }
}