...
public class WeatherStation extends Observable implements Runnable {
    ...
    private final IBarometer baro ; //Barometer

    private final long PERIOD = 1000 ;      // 1 sec = 1000 ms
    private final int KTOC = -27315 ;       // Kelvin to Celsius conversion.

    private int currentReading ;
    private double barometricReading ;

    /*
     * When a WeatherStation object is created, it in turn creates the sensor
     * objects it will use.
     */
    public WeatherStation(ITempSensor _sensor, IBarometer _baro) {
        sensor = _sensor;
        baro = _baro;
        currentReading = adapter.reading() ;
        barometricReading = baro.pressure() ;
    }
    ...
    public synchronized double getFahrenheit() {
        double cel = (currentReading + KTOC) / 100.0 ;
        return cel * 1.8 + 32 ; 
    }
    
    /*
     * Return the current pressure reading in inches.
     */
    ...
    /*
     * Return the current pressure reading in millibars.
     */
    public synchronized double getMillibars() {
        return barometricReading * 33.864 ;
    }
}
